...
namespace opkele {
	...
	if(sts=="DH-SHA1" || sts=="DH-SHA256") {
	    ...
	    try {
          string tep = inm.get_field("dh_modulus");
          dh->p = util::base64_to_bignum(tep);
	    }catch(failed_lookup& yo) {
	...
	try {
      ...
      printf("here %s", e.what());
   }
	try {
	    openid2 = (inm.get_field("ns")==OIURI_OPENID20);
	}catch(opkele::exception&) { openid2 = false; }
	try {
	    return_to = inm.get_field("return_to");
	}catch(opkele::exception&) { }
	if(openid2) {
	    try {
		realm = inm.get_field("realm");
		if(realm.empty())
		    throw opkele::failed_lookup(OPKELE_CP_ "Empty realm doesn't count");
	    }catch(opkele::exception&) {
		try {
		    realm = inm.get_field("trust_root");
		    if(realm.empty())
			throw opkele::failed_lookup(OPKELE_CP_ "Empty trust_root doesn't count");
		}catch(opkele::exception&) {
		    if(return_to.empty())
